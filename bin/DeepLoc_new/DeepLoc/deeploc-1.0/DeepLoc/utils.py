...
def encode_blosum(seq):
	"""Encode a protein sequence into BLOSUM62
	"""

	blosum = '4 -1 -2 -2  0 -1 -1  0 -2 -1 -1 -1 -1 -2 -1  1  0 -3 -2  0 -2 -1  0 \
	...
	blosum = blosum.split()
	blosum_list = list()
	for e in range(0,len(blosum),23):
		blosum_list.append(blosum[e:e+23])
	blomap = np.array(blosum_list)
	code = list("ARNDCQEGHILKMFPSTWYVBZX")
	seq_len = len(seq)
	n_feat = blomap.shape[1]
...
def parse_fasta(fastafile):
	"""Convert fasta file into arrays that are suitable to input to the neural network model
	"""
	index = 0
	ids = []
	sequences = []
	...
	try:
		...
		print("Can't open file,", error.strerror)
		sys.exit(1)
	...
	for line in f:
		if index == 0:
			if not line.startswith(">"):
				raise ValueError('The input file does not look like a fasta file')
	...
	if '' in sequences:
		raise ValueError('Error while reading fasta file, some entries have no sequence.')

	return ids, sequences

def encode_array(protein_array):
	"""Encoded a list of sorted proteins into BLOSUM62
	"""

	n = protein_array.shape[0]
	max_len = len(protein_array[0])
	n_feat = 23
	inputs_array = np.zeros((n,max_len,n_feat))
	mask_array = np.zeros((n,max_len))

	for i, protein in enumerate(protein_array):
		length = len(protein)

		if length < max_len:
			inputs_array[i,:length] = encode_blosum(protein)
			mask_array[i,:length] = np.ones(length)
		else:
			inputs_array[i] = encode_blosum(protein)
			mask_array[i] = np.ones(length)

	return inputs_array, mask_array



def iterate_minibatches(ids_array, input_array, batch_size):
	"""Create minibatches based on protein matrix
	"""


	n_seq = len(input_array)
	all_length = [len(e) for e in input_array]

	input_array = np.array(input_array)
	ids_array = np.array(ids_array)
	length_array = np.array(all_length)

	rr_sort = np.argsort(length_array)
	rr_sort = rr_sort[::-1]

	ids_sorted = ids_array[rr_sort]
	input_sorted = input_array[rr_sort]

	input_sorted, masks_sorted = encode_array(input_sorted)

	if n_seq % batch_size == 0:
		n_batch = int(n_seq / batch_size)
	else:
		n_batch = int(n_seq / batch_size) + 1
	
	index = 0
	for batch in range(n_batch):
		c_batch = batch*batch_size
		batch_ids = ids_sorted[c_batch:c_batch+batch_size]
		batch_input = input_sorted[c_batch:c_batch+batch_size]
		batch_masks = masks_sorted[c_batch:c_batch+batch_size]
		max_len_batch = int(np.sum(batch_masks[0]))
		batch_input = batch_input[:,:max_len_batch]
		batch_masks = batch_masks[:,:max_len_batch]

		yield batch_ids, batch_input.astype('float32'), batch_masks.astype('float32')


def Reweight(net_probs):
	locs = np.array([4043,2542,1973,1510,1340,862,757,356,321,154])
	locs = locs.astype('float32')
	locs_mat = np.zeros((10,10))
	
	for i in range(10):
		locs_mat[i] = locs / (locs + locs[i])
	def ret(x):
		fi = np.sum(x * locs_mat.T, axis=0)
		return fi / np.sum(fi)
	x = np.apply_along_axis(ret,1,net_probs)
	return x


def tree_probs(tree_sig):
	...
	pt_cmlys = tree_sig[:,4]
	pt_term = tree_sig[:,5]
	pt_mitchl = tree_sig[:,6]
	pt_pernc = tree_sig[:,7]
	pt_nuccyt = tree_sig[:,8]
		

	# First level, secretory or non-secretory pathway
	qt_sec = pt_sec
	qt_nosec = (1.0 - pt_sec)

	# Second level
	qt_noext = qt_sec * pt_ext
	qt_ext = qt_sec * (1.0 - pt_ext)

	qt_noterm = qt_nosec * pt_term
	qt_term = qt_nosec * (1.0 - pt_term)

	# Third level
	qt_noint = qt_noext * pt_int
	qt_int = qt_noext * (1.0 - pt_int)

	qt_per = qt_noterm * pt_pernc
	qt_noper = qt_noterm * (1.0 - pt_pernc)

	qt_mit = qt_term * pt_mitchl
	qt_chl = qt_term * (1.0 - pt_mitchl)

	# Forth level
	qt_er = qt_int * pt_ergol
	qt_gol = qt_int * (1.0 - pt_ergol)

	qt_cm = qt_noint * pt_cmlys
	qt_lys = qt_noint * (1.0 - pt_cmlys)

	qt_nuc = qt_noper * pt_nuccyt
	qt_cyt = qt_noper * (1.0 - pt_nuccyt)

	final_probs = np.vstack((qt_nuc, qt_cyt, qt_ext, qt_mit, qt_cm, qt_er, qt_chl, qt_gol, qt_lys, qt_per)) 

	return final_probs.T
...
class LSTMAttentionDecodeFeedbackLayer(MergeLayer):
	...
	def __init__(self, incoming,
				 ...
				 aln_num_units,
				 n_decodesteps,
				 W_align=init.Orthogonal(),
				 U_align=init.Orthogonal(),
				 v_align=init.Orthogonal(),
				 U_conv_align=init.Orthogonal(),
				 nonlinearity_align=nonlinearities.tanh,
				 W_hid_to_ingate=init.Orthogonal(),
				 ...
				 W_hid_to_forgetgate=init.Orthogonal(),
	...
	def get_output_for(self, inputs, **kwargs):
		...
		if self.unroll_scan:
			...
			cell_out, hid_out, alpha_out, weighted_hidden_out = scan(
				fn=step_fun,
				sequences=sequences,
				outputs_info=[cell_init, hid_init, alpha_init, weighted_hidden_init],
				go_backwards=self.backwards,
